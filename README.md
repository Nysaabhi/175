...
processFlightQuery(query) {
  ...
  // Check if the query is related to flight search
  ...
  if (match) {
    const fromCity = match[1] ? match[1].trim() : null;
    const toCity = match[2] ? match[2].trim() : null;
    ...
    const classMatch = normalizedQuery.match(/\b(economy|premium economy|business)\b/i);
    ...
    const from = fromCity ? airportData[fromCity.toLowerCase()] || fromCity : null;
    const to = toCity ? airportData[toCity.toLowerCase()] || toCity : null;

    return this.createFlightSearchResponse(from, to, departureDate, returnDate, classType, travellers);
  }
...
createFlightSearchResponse(from = null, to = null, departureDate = null, returnDate = null, classType = 'e', travellers = 1) {
  ...
  const generateCityOptions = (selectedCity) => {
    return Object.keys(airportData)
      .map(city => `<option value="${airportData[city]}" ${selectedCity === airportData[city] ? 'selected' : ''}>${city.charAt(0).toUpperCase() + city.slice(1)}</option>`)
      .join('');
  };
      ...
      <form id="flightSearchForm">
        ...
        <div class="form-group">
          ...
          <select id="to" name="to" required>
            ...
            ${generateCityOptions(to)}
          </select>
        </div>
        <div class="form-group">
          <label for="tripType">Trip Type:</label>
          <select id="tripType" name="tripType">
            <option value="one-way" ${!isRoundTrip ? 'selected' : ''}>One Way</option>
            <option value="round-trip" ${isRoundTrip ? 'selected' : ''}>Round Trip</option>
          </select>
        </div>
        <div class="form-group">
...
generateFlightSearchUrl(from, to, departureDate, returnDate, classType, travellers, tripType) {
  ...
  const params = new URLSearchParams({
    ...
    to: to.toUpperCase(),
    date: formatDate(departureDate), // Format departure date as DDMMYYYY
    returnDate: tripType === 'round-trip' ? formatDate(returnDate) : '', // Format return date as DDMMYYYY
    adults: travellers,
    children: 0,
    infants: 0,
    class: classType,
